...
class Command(BaseCommand):
    ...
    def calculate_total_today_kwh(self):
        # ---------------------------------------------------------
        ...
        tables = self.query_api.query(flux_today)
    ...
    def calculate_total_yesterday_kwh(self):
        # ---------------------------------------------------------
    ...
    def alarm_counts(self):
        ...
        tables = self.query_api.query(flux_alarms_active)
        ...
        tables = self.query_api.query(flux_alarms_high)
        high_alarm_count = 0
        for table in tables:
            for rec in table.records:
                try:
    ...
    def calculate_solar_today_kwh(self):
        # ---------------------------------------------------------
    ...
    def pct_solar_share(self, solar_kwh, total_kwh):
    ...
    def realtime_chart(self):
        ...
        tables = self.query_api.query(flux_realtime_24)
    ...
    def weekly_pln_vs_solar(self):
        ...
        tables_pln = self.query_api.query(flux_weekly_pln)
        tables_solar = self.query_api.query(flux_weekly_solar)
    ...
    def room_overview(self):
        ...
        |> sum()
        '''
            tables_dev = self.query_api.query(flux_dev)
            dev_kwh = 0.0
            temp_dev = []
        ...
        |> last()
        '''
            tables_state = self.query_api.query(flux_state)
            ac_state = None
            lamp_state = None
    ...
    def system_online_status(self):
        ...
        tables = self.query_api.query(flux_last)
        last_time = None
        for table in tables:
            for rec in table.records:
    ...
    def room_status(self):
        ...
        for dev in PM_GRID:
            ...
            |> last()
            '''
            tables_room = self.query_api.query(flux_room)
            ...
            '''
            tables_history = self.query_api.query(flux_history)
    ...
    def build_dashboard_payload(self):
        ...
        solar_share_pct = self.pct_solar_share(solar_today_kwh, total_load)
        realtime_points = self.realtime_chart()
        weekly_pln, weekly_solar = self.weekly_pln_vs_solar()
        overview = self.room_overview()
        system_online = self.system_online_status()
